...
alt.onServer("Client:AdminMenu:GetWaypointInfo", () => {
    ...
    if (game.doesBlipExist(waypoint)) {
        ...
        game.setEntityCoords(alt.Player.local.scriptID, coords.x, coords.y, 150, true, false, false, true);
        alt.setTimeout(() => {
...
alt.onServer("Client:AdminMenu:Spectate", (target, info) => {
    if (info == "on") {
        ...
        game.setEntityCoords(alt.Player.local.scriptID, target.pos.x, target.pos.y, target.pos.z - 3, true, false, false, true);
        alt.setTimeout(() => {
        ...
        spectate_lastpos = game.getEntityCoords(alt.Player.local.scriptID, true);
        game.setEntityCoords(alt.Player.local.scriptID, spectate_lastpos.x, spectate_lastpos.y, spectate_lastpos.z, true, false, false, true);
    ...
    if (info == "on") {
        nametags_tick = alt.everyTick(() => {
            for (let i = 0, n = alt.Player.all.length; i < n; i++) {
                ...
                if (!game.isEntityDead(player.scriptID, true)) {
...
function drawBar(value, lineHeight, scale, position, r, g, b, a) {
    ...
    game.drawRect((healthWidth - 100 * 0.0005 * scale) / 2, lineHeight + position * lineHeight, healthWidth, lineHeight / 4, r, g, b, a, true);
}

function drawBarBackground(value, lineHeight, scale, position, r, g, b, a) {
    const width = value * 0.0005 * scale;
    game.drawRect(0, lineHeight + position * lineHeight, width + 0.002, lineHeight / 3 + 0.002, 0, 0, 0, 255, true);
    game.drawRect(0, lineHeight + position * lineHeight, width, lineHeight / 3, r, g, b, a, true);
}
