    ...
    class ShopHandler : IScript
    {
        ...
        public async Task buyShopItem(IPlayer player, int shopId, int amount, string itemname)
        {
            ...
            if (invWeight + itemWeight <= 15f)
            {
                if (shopFaction > 0 && shopFaction != 0)
                {
                    ...
                    if (!CharactersInventory.ExistCharacterItem(charId, "Bargeld", "inventory") || CharactersInventory.GetCharacterItemAmount(charId, "Bargeld", "inventory") < itemPrice) { HUDHandler.SendNotification(player, 3, 2500, "Du hast nicht genügend Gelddabei."); return; }
            ...
            if (Characters.GetCharacterBackpack(charId) != -2 && backpackWeight + itemWeight <= Characters.GetCharacterBackpackSize(Characters.GetCharacterBackpack(charId)))
            {
                if (shopFaction > 0 && shopFaction != 0)
                {
                    ...
                    if (ServerFactions.GetCharacterFactionId(charId) != shopFaction) { HUDHandler.SendNotification(player, 3, 2500, $"Du hast hier keinen Zugriff drauf (Gefordert: {shopFaction} - Deine: {ServerFactions.GetCharacterFactionId(charId)}."); return; }
                    if (!CharactersInventory.ExistCharacterItem(charId, "Bargeld", "inventory") || CharactersInventory.GetCharacterItemAmount(charId, "Bargeld", "inventory") < itemPrice) { HUDHandler.SendNotification(player, 3, 2500, "Du hast nicht genügend Geld dabei."); return; }
                    CharactersInventory.RemoveCharacterItemAmount(charId, "Bargeld", itemPrice, "inventory");
                    LoggingService.NewFactionLog(shopFaction, charId, 0, "shop", $"{Characters.GetCharacterName(charId)} hat {itemname} ({amount}x) für {itemPrice}$ erworben.");
                }
                else
        ...
        public async Task robShop(ClassicPlayer player, int shopId)
        {
            try
            {
                ...
                if (!ServerShops.IsShopRobaable(shopId))
                {
                    HUDHandler.SendNotification(player, 3, 2500, "Dieser Shop wurde bereits vor kurzem ausgeraubt.");
                    return;
                }
                ...
                ServerFactions.AddNewFactionDispatch(shopId + 10000, 1, $"Aktiver Shopraub ({ServerShops.GetShopName(shopId)})", player.Position);
                ServerFactions.AddNewFactionDispatch(shopId + 10000, 3, $"Aktiver Shopraub ({ServerShops.GetShopName(shopId)})", player.Position);

                foreach (var p in Alt.GetAllPlayers().Where(x => x != null && x.Exists && x.GetCharacterMetaId() > 0).ToList())
                {
                    if (!ServerFactions.IsCharacterInAnyFaction((int)p.GetCharacterMetaId()) || !ServerFactions.IsCharacterInFactionDuty((int)p.GetCharacterMetaId()) || ServerFactions.GetCharacterFactionId((int)p.GetCharacterMetaId()) != 3 && ServerFactions.GetCharacterFactionId((int)p.GetCharacterMetaId()) != 1) continue;
                    HUDHandler.SendNotification(p, 1, 150000, $"Ein stiller Alarm wurde ausgelöst. ({ServerShops.GetShopName(shopId)})");
                }
                ...
                HUDHandler.SendNotification(player, 1, 600000, "Du raubst den Laden nun aus - warte 10 Minuten um das Geld zu erhalten.");
        ...
        public async Task sellShopItem(IPlayer player, int shopId, int amount, string itemname)
        {
            ...
            if (ServerFactions.IsCharacterInAnyFaction(charId) && shopId == 88)
            {
                int newPrice = Convert.ToInt32(itemSellPrice * 0.9);
                int factionRevenue = Convert.ToInt32(itemSellPrice * 0.1);
                int factionMoney = ServerFactions.GetFactionBankMoney(ServerFactions.GetCharacterFactionId(charId));
                factionMoney += amount * factionRevenue;
                ServerFactions.SetFactionBankMoney(ServerFactions.GetCharacterFactionId(charId), factionMoney);

                itemSellPrice = newPrice;
            }
        ...
        public async Task BuyVehicle(IPlayer player, int shopid, string hash)
        {
            try
            {
                ...
                if (shopid == 6)
                {
                    ...
                    {
                        ServerVehicles.CreateVehicle(fHash, charId, 0, 6, false, 1, ParkOut, RotOut, $"LSPD{rnd3}", 134, 134, 134);
                        /*                        ServerVehicles.CreateVehicle(fHash, charId, Vtyp, fID, false, gID, ParkOut, RotOut, $"LSPD{rnd3}", 134, 134);
                        */
                        HUDHandler.SendNotification(player, 2, 5000, $"Fahrzeug erfolgreich gekauft. Kennzeichen: LSPD{rnd3}");
                    }
                    else
                    {
                        ServerVehicles.CreateVehicle(fHash, charId, 0, 0, false, 1, ParkOut, RotOut, $"LSPD{rnd3}", 134, 134, 134);
                ...
                {
                    ServerVehicles.CreateVehicle(fHash, charId, 0, 1, false, 2, ParkOut, RotOut, $"LSPD{rnd3}", 134, 134, 134);
                    CharactersInventory.AddCharacterItem(charId, $"Fahrzeugschluessel LSPD{rnd3}", 2, "schluessel");
                    HUDHandler.SendNotification(player, 2, 5000, $"Fahrzeug erfolgreich gekauft. Kennzeichen: LSPD{rnd3}");
                }
                ...
                {
                    ServerVehicles.CreateVehicle(fHash, charId, 0, 4, false, 4, ParkOut, RotOut, $"MD{rnd3}", 134, 134, 134);
                ...
                {
                    ServerVehicles.CreateVehicle(fHash, charId, 0, 4, false, 5, ParkOut, RotOut, $"MD{rnd3}", 134, 134, 134);
                    CharactersInventory.AddCharacterItem(charId, $"Fahrzeugschluessel MD{rnd3}", 2, "schluessel");
                    HUDHandler.SendNotification(player, 2, 5000, $"Fahrzeug erfolgreich gekauft. Kennzeichen: MD{rnd3}");
                }
                else if (shopid == 10)
                {
                    ServerVehicles.CreateVehicle(fHash, charId, 0, 5, false, 8, ParkOut, RotOut, $"LSC{rnd3}", 134, 134, 134);
                ...
                {
                    ServerVehicles.CreateVehicle(fHash, charId, 0, 6, false, 9, ParkOut, RotOut, $"LSF{rnd3}", 134, 134, 134);
                ...
                {
                    ServerVehicles.CreateVehicle(fHash, charId, 0, 2, false, 9, ParkOut, RotOut, $"FIB{rnd3}", 134, 134, 134);
                ...
                {
                    ServerVehicles.CreateVehicle(fHash, charId, 0, 0, false, 10, ParkOut, RotOut, $"LS{rnd}", 134, 134, 134);
        ...
        public async Task BuyVehicleBank(IPlayer player, int shopid, string hash)
        {
            try
            {
                ...
                if (shopid == 1)
                {
                    if (hash == "2046537925" && ServerFactions.GetCharacterFactionRank(charId) >= 15)
                    {
                        ServerVehicles.CreateVehicle(fHash, charId, 0, 1, false, 1, ParkOut, RotOut, $"LSPD{rnd3}", 134, 134, 134);
                        HUDHandler.SendNotification(player, 2, 5000, $"Fahrzeug erfolgreich gekauft. Kennzeichen: LSPD{rnd3}");
                    }
                    else
                    {
                ...
                {
                    ServerVehicles.CreateVehicle(fHash, charId, 0, 9, false, 7, ParkOut, RotOut, $"VUC{rnd3}", 134, 134, 134);
                ...
                {
                    ServerVehicles.CreateVehicle(fHash, charId, 0, 0, false, 10, ParkOut, RotOut, $"LS-{rnd2}", 134, 134, 134);
        ...
        internal static void SellVehicle(IPlayer player, string shopname, int shopId)
        {
            ...
            ulong fHash = Convert.ToUInt64(hash);
            int price = ServerVehicleShops.GetVehicleShopPrice2(fHash) / 100 * 70; //FIXEN AMK!! --> D O N E
            if (ServerVehicleShops.GetVehicleShopId(shopId) != 1000)
            {
                if (p == null || ServerVehicles.GetVehicleOwner(veh) != p.CharacterId) { HUDHandler.SendNotification(player, 4, 7500, $"Dies ist nicht dein Fahrzeug!"); return; }
