    ...
    class HouseHandler : IScript
    {
        ...
        public async Task StorageItem(IPlayer player, int houseId, string itemName, int itemAmount, string fromContainer)
        {
            try
            {
                ...
                DiscordLog.SendEmbed("housestorage", "Hauslager Log", $"{ Characters.GetCharacterName(User.GetPlayerOnline(player))} hat den Gegenstand {itemName} {itemAmount}x in das Hauslager gelagert.");
                //ToDo: Log Eintrag
            }
            catch (Exception e)
            {
