    ...
    class TabletHandler : IScript
    {
        ...
        public async Task BankingAppNewTransaction(IPlayer player, int targetAccountNumber, string transactionMessage, int moneyAmount)
        {
            try
            {
                ...
                if (!CharactersBank.ExistBankAccountNumber(targetAccountNumber)) { HUDHandler.SendNotification(player, 3, 5000, $"Die angegebene Kontonummer existiert nicht"); return; }
        ...
        public async Task DeleteFactionDispatch(IPlayer player, int factionId, int senderId)
        {
            try
            {
                if (player == null || !player.Exists || factionId <= 0) return;
        ...
        public void sendDispatchToFaction(IPlayer player, int factionId, string msg)
        {
            try
            {
                ...
                if (ServerFactions.ExistDispatchBySender(charId)) { ServerFactions.RemoveDispatchWithoutFactionId((int)player.GetCharacterMetaId()); HUDHandler.SendNotification(player, 4, 3500, "Vorheriger Dispatch wurde entfernt."); }
