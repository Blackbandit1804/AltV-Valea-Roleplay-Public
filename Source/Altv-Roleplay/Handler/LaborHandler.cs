...
namespace Altv_Roleplay.Handler
{
    class LaborHandler : IScript
    {
        ...
        public async Task switchItemToLabor(ClassicPlayer player, string itemName, int itemAmount)
        {
            ...
            if ((itemName == "Batteriezellen" && ServerFactions.GetLaborItemAmount(factionId, User.GetPlayerOnline(player), "Batteriezellen") + itemAmount > 200) || (itemName == "Hanfsamenpulver" && ServerFactions.GetLaborItemAmount(factionId, User.GetPlayerOnline(player), "Hanfsamenpulver") + itemAmount > 200) || (itemName == "Dünger" && ServerFactions.GetLaborItemAmount(factionId, User.GetPlayerOnline(player), "Dünger") + itemAmount > 200) || (itemName == "Ephedrinpulver" && ServerFactions.GetLaborItemAmount(factionId, User.GetPlayerOnline(player), "Ephedrinpulver") + itemAmount > 50) || (itemName == "Toilettenreiniger" && ServerFactions.GetLaborItemAmount(factionId, User.GetPlayerOnline(player), "Toilettenreiniger") + itemAmount > 50))
            {
                HUDHandler.SendNotification(player, 2, 1500, $"Soviele Gegenstände kannst du nicht einlagern, maximal 200 von jedem (aktuell: {ServerFactions.GetLaborItemAmount(factionId, User.GetPlayerOnline(player), itemName)}x {itemName}).");
                return;
            }
