    ...
    class FactionHandler : IScript
    {
        ...
        public async Task FactionStorageStorageItem(IPlayer player, int factionId, int charId, string itemName, int amount, string fromContainer)
        {
            try
            {
                ...
                if (factionId < 8 && !ServerFactions.IsCharacterInFactionDuty(charId)) { HUDHandler.SendNotification(player, 4, 5000, "Fehler: Du bist nicht im Dienst."); return; }
                ...
                HUDHandler.SendNotification(player, 2, 5000, $"Du hast {itemName} ({amount}x) in den Spind gelegt.");
