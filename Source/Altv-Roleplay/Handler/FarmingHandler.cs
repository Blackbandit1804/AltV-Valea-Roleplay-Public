...
namespace Altv_Roleplay.Handler
{
    class FarmingHandler : IScript
    {
        ...
        public static void openFarmingCEF(IPlayer player, string neededItem, string producedItem, int neededItemAmount, int producedItemAmount, int duration, string neededItemTWO, string neededItemTHREE, int neededItemTWOAmount, int neededItemTHREEAmount)
        {
            try
            {
                player.EmitLocked("Client:Farming:createCEF", ServerItems.GetItemObject(neededItem), ServerItems.GetItemObject(producedItem), neededItemAmount, producedItemAmount, duration, ServerItems.GetItemObject(neededItemTWO), ServerItems.GetItemObject(neededItemTHREE), neededItemTWOAmount, neededItemTHREEAmount);
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        internal static async void ProduceItem(IPlayer player, string neededItem, string producedItem, int neededItemAmount, int producedItemAmount, int duration, string neededItemTWO, string neededItemTHREE, int neededItemTWOAmount, int neededItemTHREEAmount)
        {
            try
            {
                ...
                switch (hasItemSlot)
                {
                    case 2 when !CharactersInventory.ExistCharacterItem(charId, neededItemTWO, "inventory") && !CharactersInventory.ExistCharacterItem(charId, neededItemTWO, "backpack"):
                        HUDHandler.SendNotification(player, 4, 5000, $"Du hast nicht die richtigen Gegenstände, um " + neededItem + " zu verarbeiten."); return;
                    case 3 when !CharactersInventory.ExistCharacterItem(charId, neededItemTHREE, "inventory") && !CharactersInventory.ExistCharacterItem(charId, neededItemTHREE, "backpack"):
                        HUDHandler.SendNotification(player, 4, 5000, $"Du hast nicht die richtigen Gegenstände, um " + neededItem + " zu verarbeiten."); return;
                        break;
                }
                ...
                if (finalAmount < neededItemAmount || finalAmount < neededItemTWOAmount || finalAmount < neededItemTHREEAmount)
                {
                    HUDHandler.SendNotification(player, 3, 5000,
                        $"Du hast nicht die nötigen Gegenstände dabei, um {neededItem} zu verarbeiten!");
                    return;
