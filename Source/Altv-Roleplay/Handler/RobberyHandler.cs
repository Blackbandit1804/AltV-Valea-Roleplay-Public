    ...
    class RobberyHandler : IScript
    {
        ...
        internal static void pickUpBankGold(ClassicPlayer player, bankPickUpPosition bankRobPosGold)
        {
            try
            {
                ...
                int randomAnzahl = new Random().Next(45000, 65000);
        ...
        internal static async Task breakUpBank(ClassicPlayer player)
        {
            ...
            {
                if (player == null || !player.Exists || player.CharacterId <= 0) return;
                if (ServerFactions.GetFactionDutyMemberCount(1) + ServerFactions.GetFactionDutyMemberCount(3) < 8) // Wider (1) auf 4 machen
                {
                    HUDHandler.SendNotification(player, 4, 2500, "Es sind nicht genug Beamte im Dienst.");
                    return;
                }
                ...
                if (!CharactersInventory.ExistCharacterItem2((int)player.GetCharacterMetaId(), "Schweißgerät")) { HUDHandler.SendNotification(player, 3, 2000, "Du hast nicht das benötigte Item."); return; }
                ...
                ServerFactions.AddNewFactionDispatch(0, 1, $"Aktiver Staatsbank Raub", player.Position);
                ServerFactions.AddNewFactionDispatch(0, 3, $"Aktiver Staatsbank Raub", player.Position);
                ...
                foreach (var PDmember in Alt.GetAllPlayers().ToList().Where(x => x != null && x.Exists && x.GetCharacterMetaId() > 0 && ServerFactions.IsCharacterInAnyFaction((int)x.GetCharacterMetaId()) && ServerFactions.GetCharacterFactionId((int)x.GetCharacterMetaId()) == 1 || ServerFactions.GetCharacterFactionId((int)x.GetCharacterMetaId()) == 3))
                    HUDHandler.SendNotification(PDmember, 3, 30000, "Ein Einbruch in die Staatsbank wurde gemeldet.");
                ...
                CharactersInventory.RemoveCharacterItemAmount2((int)player.GetCharacterMetaId(), "Schweißgerät", 1);
        ...
        internal static async Task breakUpATM(ClassicPlayer player, Position atmpos)
        {
            try
            {
                ...
                if (ServerFactions.GetFactionDutyMemberCount(1) < 2) // Wider (1) auf 4 machen
                {
                    HUDHandler.SendNotification(player, 4, 2500, "Es sind weniger als 2 Polizisten im Staat.");
                    return;
                }
                ...
                HUDHandler.SendNotification(player, 1, 24000, "Du schweißt den Bankautomaten auf...");
                player.EmitLocked("Client:Inventory:PlayAnimation", "amb@world_human_welding@male@idle_a", "idle_a", 240000, 1, false); //Animation 

                foreach (var PDmember in Alt.GetAllPlayers().ToList().Where(x => x != null && x.Exists && x.GetCharacterMetaId() > 0 && ServerFactions.IsCharacterInAnyFaction((int)x.GetCharacterMetaId()) && ServerFactions.GetCharacterFactionId((int)x.GetCharacterMetaId()) == 1))
                    HUDHandler.SendNotification(PDmember, 3, 30000, "Ein Bankautomat meldet einen stillen Alarm!!.");
                ServerFactions.AddNewFactionDispatch(0, 1, "Ein Bankautomat meldet einen stillen Alarm.", atmpos);
                ServerFactions.AddNewFactionDispatch(0, 3, "Ein Bankautomat meldet einen stillen Alarm.", atmpos);
                ...
                foreach (var PDmember in Alt.GetAllPlayers().ToList().Where(x => x != null && x.Exists && x.GetCharacterMetaId() > 0 && ServerFactions.IsCharacterInAnyFaction((int)x.GetCharacterMetaId()) && ServerFactions.IsCharacterInFactionDuty((int)player.GetCharacterMetaId()) && (ServerFactions.GetCharacterFactionId((int)x.GetCharacterMetaId()) == 1) || ServerFactions.GetCharacterFactionId((int)x.GetCharacterMetaId()) == 3))
                    HUDHandler.SendNotification(PDmember, 3, 5000, "Der Bankautomat wurde aufgebrochen!");
                ServerFactions.AddNewFactionDispatch(0, 1, $"Der Bankautomat wurde aufgebrochen! Es wurde {rnd}$ gestohlen!", atmpos);
        ...
        */
        ...
        public static async Task robJewelery(ClassicPlayer player)
        {
            ...
            {
                if (player == null || !player.Exists || player.CharacterId <= 0) return;
                if (ServerFactions.GetFactionDutyMemberCount(1) + ServerFactions.GetFactionDutyMemberCount(3) < 6) // Wider (1) auf 4 machen
                {
                    HUDHandler.SendNotification(player, 4, 2500, "Es sind nicht genug Beamte im Dienst.");
                    return;
                }
                ...
                foreach (var PDmember in Alt.GetAllPlayers().ToList().Where(x => x != null && x.Exists && x.GetCharacterMetaId() > 0 && ServerFactions.IsCharacterInAnyFaction((int)x.GetCharacterMetaId()) && ServerFactions.GetCharacterFactionId((int)x.GetCharacterMetaId()) == 1 || ServerFactions.GetCharacterFactionId((int)x.GetCharacterMetaId()) == 3))
                    HUDHandler.SendNotification(PDmember, 3, 30000, "Ein Einbruch in den Juwelier wurde gemeldet.");
                ServerFactions.AddNewFactionDispatch(0, 1, $"Aktiver Juwelier Raub", player.Position);
                ServerFactions.AddNewFactionDispatch(0, 3, $"Aktiver Juwelier Raub", player.Position);
        ...
        public static async Task washmoney(IPlayer player)
        {
            try
            {
                ...
                HUDHandler.SendNotification(player, 2, 50000, "Das Geldwaschen startet nun... (50s)");

                await Task.Delay(50000); //Zeit bis offen = 50s
                if (!player.Position.IsInRange(Constants.Positions.Schwarzwasch, 5f))
                {
                    HUDHandler.SendNotification(player, 4, 5000, "Du hast dich vom Geldwaschen entfernt - Abbruch");
                    return;
                }
