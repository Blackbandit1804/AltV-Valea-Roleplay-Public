...
namespace Altv_Roleplay.Handler
{
    class GarageHandler : IScript
    {
        internal static void OpenGarageCEF(IPlayer player, int garageId)
        {
            try
            {
                ...
                if (garageInfo.name.Contains("Fraktion"))
                {
                    ...
                    inString = GetGarageParkInString(player, garageSlots, charId, garageId, true, factionCut, charFaction); //Array von Fahrzeugen die um die Garage rum zum Einparken stehen
                    outString = GetGarageParkOutString(player, garageId, charId, true, factionCut);
        ...
        public static string GetGarageParkInString(IPlayer player, IReadOnlyCollection<Server_Garage_Slots> garageSlots, int charId, int garageId, bool isFaction, string factionShort, int factionId)
        {
            ...
            {
                bool hasKey = isFaction switch
                {
                    false => CharactersInventory.ExistCharacterItem(charId, "Fahrzeugschluessel " + veh.NumberplateText,
                        "schluessel"),
                    true => ServerFactions.GetCharacterFactionId(charId) == ServerVehicles.GetVehicleFactionId2((int)veh.GetVehicleId())
                };
                bool isOwner = ServerVehicles.GetVehicleOwner(veh) == charId;
        ...
        public static string GetGarageParkOutString(IPlayer player, int garageId, int charId, bool isFaction, string factionShort)
        {
            try
            {
                ...
                {
                    inGarageVehs = ServerVehicles.ServerVehicles_.Where(v => v.isInGarage && v.garageId == garageId).ToList();
                }
                ...
                foreach (var vehicle in inGarageVehs)
                {
                    bool hasKey = isFaction switch
                    {
                        false => CharactersInventory.ExistCharacterItem(charId, "Fahrzeugschluessel " + vehicle.plate,
                            "schluessel"),
                        true => ServerFactions.GetCharacterFactionId(charId) == ServerVehicles.GetVehicleFactionId2(vehicle.id)
                    };
                    bool isOwner = vehicle.charid == charId;
                    if (!hasKey && !isOwner) continue;
        ...
        public async Task DoGarageAction(IPlayer player, int garageid, string action, int vehID)
        {
            try
            {
                ...
                if (action == "storage")
                {
                    ...
                    if (ServerVehicles.GetVehicleFactionId(vehicle) != 0)
                    {
                        if (ServerVehicles.GetVehicleFactionId(vehicle) <= 8)
                        {
                            if (!ServerFactions.IsCharacterInFactionDuty((int)player.GetCharacterMetaId()))
                            {
                                HUDHandler.SendNotification(player, 2, 5000, $"Dieses Fahrzeug gehört nicht dir.");
                                DiscordLog.SendEmbed("sus", "OOC-Nachricht", $"{player.Name} [{(int)player.GetCharacterMetaId()}] hat probiert ein Fahrzeug einzuparken, welches ihm nicht gehört. (blocked)");
                                return;
                            }
                        }
                        if (!ServerFactions.IsCharacterInAnyFaction((int)player.GetCharacterMetaId()) || ServerVehicles.GetVehicleFactionId(vehicle) != ServerFactions.GetCharacterFactionId((int)player.GetCharacterMetaId()))
                        {
                            HUDHandler.SendNotification(player, 2, 5000, $"Dieses Fahrzeug gehört nicht dir.");
                            DiscordLog.SendEmbed("sus", "OOC-Nachricht", $"{player.Name} [{(int)player.GetCharacterMetaId()}] hat probiert ein Fahrzeug einzuparken, welches ihm nicht gehört. (blocked)");
                            return;
                    ...
                    else if (!CharactersInventory.ExistCharacterItem((int)player.GetCharacterMetaId(), $"Fahrzeugschluessel {vehicle.NumberplateText}", "schluessel"))
                    {
                        HUDHandler.SendNotification(player, 2, 5000, $"Dieses Fahrzeug gehört nicht dir.");
                        DiscordLog.SendEmbed("sus", "OOC-Nachricht", $"{player.Name} [{(int)player.GetCharacterMetaId()}] hat probiert ein Fahrzeug einzuparken, welches ihm nicht gehört. (blocked)");
                        return;
                ...
                else if (action == "take")
                {
                    ...
                    if (finalVeh.faction != 0)
                    {
                        if (finalVeh.faction <= 8)
                        {
                            if (!ServerFactions.IsCharacterInFactionDuty((int)player.GetCharacterMetaId()))
                            {
                                HUDHandler.SendNotification(player, 2, 5000, $"Dieses Fahrzeug gehört nicht dir.");
                        ...
                        if (!ServerFactions.IsCharacterInAnyFaction((int)player.GetCharacterMetaId()) || finalVeh.faction != ServerFactions.GetCharacterFactionId((int)player.GetCharacterMetaId()))
                        {
                            HUDHandler.SendNotification(player, 2, 5000, $"Dieses Fahrzeug gehört nicht dir.");
                            DiscordLog.SendEmbed("sus", "OOC-Nachricht", $"{player.Name} [{(int)player.GetCharacterMetaId()}] hat probiert ein Fahrzeug einzuparken, welches ihm nicht gehört. (blocked)");
                            return;
                    ...
                    else if (!CharactersInventory.ExistCharacterItem((int)player.GetCharacterMetaId(), $"Fahrzeugschluessel {finalVeh.plate}", "schluessel"))
                    {
                        HUDHandler.SendNotification(player, 2, 5000, $"Dieses Fahrzeug gehört nicht dir.");
                        DiscordLog.SendEmbed("sus", "OOC-Nachricht", $"{player.Name} [{(int)player.GetCharacterMetaId()}] hat probiert ein Fahrzeug einzuparken, welches ihm nicht gehört. (blocked)");
                        return;
