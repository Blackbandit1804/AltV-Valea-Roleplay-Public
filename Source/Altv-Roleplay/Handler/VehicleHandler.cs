    ...
    class VehicleHandler : IScript
    {
        ...
        public async Task VehicleTrunkStorageItem(ClassicPlayer player, int vehId, int charId, string itemName, int itemAmount, string fromContainer, string type)
        {
            try
            {
                ...
                if (type == "trunk")
                {
                    DiscordLog.SendEmbed("kofferraum", "Kofferraum Log", $"{ Characters.GetCharacterName(User.GetPlayerOnline(player))} hat den Gegenstand {itemName} {itemAmount}x in den Kofferraum gelegt.");
        ...
        [ClientEvent("Server:VehicleTrunk:Close")]
        public Task CloseVehicleTrunk(ClassicPlayer player, int vehId)
        {
            // Find vehicle 
            var pool = Alt.GetAllVehicles().Cast<ClassicVehicle>();

            player.OpenTrunkId = -1;

            foreach (var veh in pool)
            {
                if (veh.VehicleId != vehId) continue;
                veh.TrunkOccupied = false;
                Alt.Log("Successfully closed!");
                break;
            }
        ...
        public async Task VehicleTrunkTakeItem(ClassicPlayer player, int vehId, int charId, string itemName, int itemAmount, string type)
        {
            try
            {
                ...
                if (type == "trunk")
                {
                    DiscordLog.SendEmbed("kofferraum", "Kofferraum Log", $"{ Characters.GetCharacterName(User.GetPlayerOnline(player))} hat den Gegenstand {itemName} {itemAmount}x aus dem Kofferraum genommen.");
        ...
        public async Task LicensingAction(IPlayer player, string action, int vehId, string vehPlate, string newPlate)
        {
            try
            {
                ...
                if (action == "anmelden")
                {
                    var notAllowedStrings = new[] { "LSPD", "LS", "GU", "PLTH", "MD", "LSC", "LSF", "DOJ", "GOV", "VUC", "LSPD-", "GU-", "PLTH-", "MD-", "LSC-", "LSF-", "DOJ-", "GOV-", "VUC-", "PL-", "SWAT", "S.W.A.T", "SWAT-", "NOOSE", "N.O.O.S.E", "FIB", "FIB-" };
        ...
        public void SetVehicleHasMutedSirensForAll(ClassicPlayer player, int vehId, bool state, bool isFromAlt)
        {
            try
            {
                var xlmp = player;
        ...
        public Task OnPlayerDisconnect(ClassicPlayer player, string reason)
        {
            // No Trunk open
            if (player.OpenTrunkId == -1) return Task.CompletedTask;
            ...
            foreach (var veh in pool)
            {
                if (veh.VehicleId != player.OpenTrunkId) continue;
                veh.TrunkOccupied = false;
                return Task.CompletedTask;
            }
