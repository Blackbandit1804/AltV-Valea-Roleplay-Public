    ...
    class Functions : IScript
    {
        ...
        public void GiveWeaponLicense(IPlayer player, string targetCharName)
        {
            try
            {
                ...
                if (ServerFactions.GetCharacterFactionId(charId) != 2) { HUDHandler.SendNotification(player, 4, 5000, "Du bist kein Angehöriger der Justiz."); return; }
        ...
        public void ViewBankTransactions(IPlayer player, int accNumber)
        {
            try
            {
                ...
                if (bankPapers == "[]" || bankPapers == "") { HUDHandler.SendNotification(player, 3, 5000, "Dieses Konto besitzt keine aktuellen Transaktionen."); return; }
