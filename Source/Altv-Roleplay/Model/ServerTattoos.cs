    ...
    class ServerTattoos
    {
        ...
        public static async void GetAllTattoos(ClassicPlayer player)
        {
            try
            {
                ...
                {
                    var skip = i * 30;
                    await player.EmitAsync("Client:TattooShop:sendItemsToClient", System.Text.Json.JsonSerializer.Serialize(tattooItems.Skip(skip).Take(30).ToList()));
                }
                if (rest != 0) await player.EmitAsync("Client:TattooShop:sendItemsToClient", System.Text.Json.JsonSerializer.Serialize(tattooItems.Skip((int)iterations * 30).ToList()));
            }
            catch (Exception e)
            {
                Console.WriteLine($"{e}");
