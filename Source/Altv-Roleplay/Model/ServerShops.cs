    ...
    class ServerShops
    {
        ...
        public static void CreateServerShop(IPlayer client, int shopid, string name, Position pos)
        {
            ...
            var ServerShopData = new Server_Shops
            {
                ...
                neededLicense = "None",
                isOnlySelling = false,
                lastRob = new DateTime(2000, 01, 01, 00, 00, 00)
            };
        ...
        public static bool IsShopRobaable(int shopId)
        {
            try
            {
                var shop = ServerShops_.ToList().FirstOrDefault(x => x.shopId == shopId);
                if (shop == null) return false;
                DateTime expDate = shop.lastRob.AddHours(2);

                if (expDate < DateTime.Now) return true;
                return false;
            }
            catch (Exception e)
            {
